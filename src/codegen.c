...
ValueType *handleType(ASTNode *type) {
  if (type->kind == AST_TYPE) {
    ...
    ValueType *valtype = malloc(sizeof(ValueType));
    valtype->base_type = NULL;
    valtype->type = LLVMArrayType(handleType(type->u.arraytype.base_type)->type,
...
LLVMValueRef codegen_expr(ASTNode *e) {
  ...
  case AST_FN_CALL: {
    ...
    if (LLVMGetTypeKind(LLVMGetReturnType(callee->type)) == LLVMVoidTypeKind) {
      LLVMBuildCall2(builder, callee->type, callee->value, args, arg_count, "");
      return NULL;
    }
    LLVMValueRef result = LLVMBuildCall2(builder, callee->type, callee->value,
  ...
  case AST_BINOP: {
    ...
    if (e->u.bin.lhs->kind == AST_FLOAT || e->u.bin.rhs->kind == AST_FLOAT) {
      if (e->u.bin.op == '+')
  ...
  case AST_ASSIGN: {
    ...
    if (!val) {
      fprintf(stderr, "Error: codegen failed on; %s", e->u.assign.name);
      exit(1);
    }
  ...
  case AST_CON_DECL: {
    ...
    if (!init) {
      fprintf(stderr, "Error: codegen failed on; %s", e->u.vardecl.name);
      exit(1);
    }
  ...
  case AST_STRING_LITERAL: {
    ...
    LLVMSetInitializer(
        global, LLVMConstStringInContext(ctx, e->u.stringval, len - 1, 0));
  ...
  case AST_ARRAY_LITERAL: {
    ...
    for (int i = 1; i < n; i++) {
      LLVMValueRef value = codegen_expr(e->u.arraylit.elements->nodes[i]);
  ...
  case AST_ARRAY_INDEX: {
    ...
    LLVMValueRef gep = LLVMBuildInBoundsGEP2(builder, array_ty, array_ptr,
                                             indices, 2, "elem_ptr");
    return LLVMBuildLoad2(builder, LLVMGetElementType(array_ty), gep,
                          "elem_val");
  }
  case AST_EQ: {
    return LLVMBuildICmp(builder, LLVMIntEQ, codegen_expr(e->u.comp.lhs),
                         codegen_expr(e->u.comp.rhs), "cmpeq");
  }
  case AST_NE: {
    return LLVMBuildICmp(builder, LLVMIntNE, codegen_expr(e->u.comp.lhs),
                         codegen_expr(e->u.comp.rhs), "cmpne");
  }
  case AST_LT: {
    return LLVMBuildICmp(builder, LLVMIntSLT, codegen_expr(e->u.comp.lhs),
                         codegen_expr(e->u.comp.rhs), "cmplt");
  }
  case AST_LE: {
    return LLVMBuildICmp(builder, LLVMIntSLE, codegen_expr(e->u.comp.lhs),
                         codegen_expr(e->u.comp.rhs), "cmple");
  }
  case AST_GT: {
    return LLVMBuildICmp(builder, LLVMIntSGT, codegen_expr(e->u.comp.lhs),
                         codegen_expr(e->u.comp.rhs), "cmpgt");
  }
  case AST_GE: {
    return LLVMBuildICmp(builder, LLVMIntSGE, codegen_expr(e->u.comp.lhs),
...
LLVMValueRef declare_extern_func(ASTNode *fn) {
  ...
  LLVMValueRef function = LLVMAddFunction(mod, fn->u.externfn.name, func_type);
